...
import { Model, isModule, isLocalEntity, Attribute, LocalEntity, Configuration } from "../../../shared/ast.js";
...
export class ModuleGenerator implements IGenerator {
  generate(model: Model, targetFolder: string): void {
    ...
    if (model.configuration){
      const package_name_application = `${package_path}.service.${model.configuration.name?.toLowerCase()}.application`;
      const APPLICATION_PATH = createPath(targetFolder, "src/main/java/", package_name_application.replaceAll(".","/"));
    ...
    for (const mod of modules) {
      const package_name = `${package_path}.service.${model.configuration?.name?.toLowerCase()}.${mod.name.toLowerCase()}`;
      const MODULE_PATH = createPath(targetFolder, "src/main/java/", package_name.replaceAll(".", "/"));
...
function generateRecordAtribute(attribute: Attribute): Generated {
  return expandToString`${capitalizeString(toString(generateTypeAttribute(attribute)))} ${attribute.name}`;
}

function generateTypeAttribute(attribute: Attribute): Generated {
  return attribute.type.toString().toLowerCase() === "date" ? "LocalDate" : attribute.type;
}
  ...
  return expandToStringWithNL`
    ...
    public class ${cls.name}Controller {
      ...
      public ${cls.name} update${cls.name}(@Argument Long id, @Argument ${cls.name}Input input) {
        ...
        if (instance == null) throw new RuntimeException("${cls.name} not found");
